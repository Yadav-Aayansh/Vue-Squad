<script setup>
    ...
    const city = ref(null)
    const is_ban = ref('')
    const profile_verified = ref('')
  
    const updateProfessionals = async () => {
      const query = `pincode=${pincode.value}&service_type=${service_type.value}&experience=${experience.value}&fee_min=${fee_min.value}&fee_max=${fee_max.value}&sort_by=${sort_by.value}&availability=${availability.value}&profile_verified=${profile_verified.value}&is_ban=${is_ban.value}`
    ...
    const actionProfessional = ref(null)
    const openActionModal = (professional) => {
        isActionOpen.value = true
        actionProfessional.value = professional
    }


    const action = async () => {
        const pro = actionProfessional.value
        const response = await fetch(`${apiUrl}/servicer/${pro.service_professional_id}?verification=${!pro.profile_verified}`, {
            method: 'PUT',
            credentials: 'include',
            headers: {
     ...
     // Action
     const isActionOpen2 = ref(false)
    const actionProfessional2 = ref(null)
    const openActionModal2 = (professional) => {
        isActionOpen2.value = true
        actionProfessional2.value = professional
    }


    const action2 = async () => {
        const pro = actionProfessional2.value
        const response = await fetch(`${apiUrl}/servicer/${pro.service_professional_id}?account=${!pro.is_ban}`, {
            method: 'PUT',
            credentials: 'include',
            headers: {
                'Content-Type': 'application/json',
  ...
  <template>
    <div class="container-fluid">
                ...
                <form @submit.prevent="updateProfessionals()" method="post">
                  ...
                  <div class="form-group mb-3">
                    ...
                    <select class="form-select rounded-pill" v-model="service_type" name="service" required
                      ...
                      <option v-for="ser in services" :key="ser.service_id" :value="ser.name">
                        {{ ser.name }}
                      </option>
                    </select>
                  </div>
                  ...
                  <div class="form-group mb-3">
                    <label for="profile_verified" class="fw-semibold">Verification Status</label>
                    <select class="form-select rounded-pill" v-model="profile_verified">
                  ...
                  <div class="form-group mb-3">
                    <label for="is_ban" class="fw-semibold">Account Status</label>
                    <select class="form-select rounded-pill" v-model="is_ban">
            ...
            <div class="card-one mb-4" v-for="professional in service_professionls">
                ...
                <div class="d-flex flex-row align-items-start position-relative">
                  <div class="d-flex flex-column align-items-center p-3">
                    ...
                    <a :href="`${staticUrl}/profile/identity/${professional.identity_proof}`" target="_blank" class="btn btn-md book_now_btn" type="button"><i class="ri-eye-line me-2"></i>View Document</a>
                    <button class="btn btn-md ban_btn" @click="openActionModal2(professional)" ><i class="ri-indeterminate-circle-line me-1"></i>
                      {{ professional.is_ban ? 'Unban' : 'Ban ' }}
                      Professional</button>
                  </div>
                  ...
                  <div class="p-3 flex-grow-1 position-relative">
                    <div class="p-4 rounded-xl cus-card2 bg-gradient">
                      ...
                      <div class="d-flex justify-content-between">
                        <div class="text-muted mb-2">
                            <i class="ri-record-circle-line me-2"></i>
                            <span class="fw-semibold">Verification: </span>
                            <strong v-if="professional.profile_verified"> <i class="ri-verified-badge-line"></i> </strong>
                            <strong v-else> <i class="ri-close-circle-line"></i> </strong>
                        </div>

                        <button class="btn btn-md verify_btn" @click="openActionModal(professional)">
                            {{ professional.profile_verified ? 'Unverify' : 'Verify ' }}
                        </button>
                    </div>
      ...
      <!-- Mobile Filter Modal -->
      <div class="modal fade hola" id="filterModal" tabindex="-1" aria-labelledby="filterModalLabel" aria-hidden="true">
    ...
    <!-- Popup Action Modal -->
    <Popup :isOpen="isActionOpen" title="Account Verification Status" @close="isActionOpen=false" :showConfirm="true"
    confirmButton="red" buttonName="Yes, Confirm" :myfunction="action">
    <p>Are you sure you want to change this professional's verification status?</p>
    </Popup>

     <!-- Popup Action2 Modal -->
     <Popup :isOpen="isActionOpen2" title="Account Status" @close="isActionOpen2=false" :showConfirm="true"
     confirmButton="red" buttonName="Yes, Confirm" :myfunction="action2">
     <p>Are you sure you want to change this professional's account status?</p>
</Popup>
  </template>
  ...
  <style scoped>
    ...
    .verify_btn {
        color: rgb(68, 68, 68);
        font-weight: 700;
        background-color: rgba(85, 189, 51, 0.5);
        border-color: black;
    }
    ...
    .verify_btn:hover {
        color: white;
        background-color: rgba(85, 189, 51, 0.76);
    }
